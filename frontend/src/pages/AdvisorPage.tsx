...
function AdvisorResult({ response }: { response: AdvisorResponse }) {
  ...
  return <section className="space-y-4"><article className={`rounded-xl border p-6 shadow-sm ${unavailable ? 'border-amber-200 bg-amber-50' : 'border-cyan-200 bg-cyan-50'}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-base font-semibold text-slate-900">Advisor response</h3><p className="mt-1 text-sm text-slate-600">Dataset ID {response.dataset_id} · Provider: {response.provider}{response.model ? ` / ${response.model}` : ''}</p></div><span className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-slate-700">{response.status.replace('_', ' ')}</span></div>{response.answer ? <NarrativeAnswer answer={response.answer} /> : <p className="mt-4 text-sm leading-6 text-slate-700">{response.message}</p>}{unavailable && <p className="mt-3 text-xs text-slate-600">The response above is a deterministic, evidence-grounded narrative. Optional AI synthesis is unavailable.</p>}</article><article className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><h3 className="text-base font-semibold text-slate-900">Analysis performed by</h3><div className="mt-3 flex flex-wrap gap-2">{response.specialists_invoked.length ? response.specialists_invoked.map((specialist) => <span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-700" key={specialist}>{specialist}</span>) : <span className="text-sm text-slate-600">No specialist analysis was needed.</span>}</div><h3 className="mt-5 text-base font-semibold text-slate-900">Tools used</h3><div className="mt-3 flex flex-wrap gap-2">{response.tools_used.length ? response.tools_used.map((tool) => <span className="rounded-full bg-cyan-50 px-3 py-1.5 text-xs font-semibold text-teal" key={tool}>{tool.replace('_', ' ')}</span>) : <span className="text-sm text-slate-600">No tools were used.</span>}</div></article><article className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><h3 className="text-base font-semibold text-slate-900">Key evidence</h3><p className="mt-1 text-sm text-slate-600">ACTUAL values are observed data, FORECAST values come from the stored model run, and HYPOTHETICAL values come from scenario analysis.</p><div className="mt-4 space-y-3">{response.evidence.map((evidence) => <EvidenceCard evidence={evidence} key={evidence.tool} />)}</div></article></section>
}

function NarrativeAnswer({ answer }: { answer: string }) {
  const headings = new Set(['SUMMARY', 'WHAT THE DATA SHOWS', 'WHY IT MATTERS', 'RECOMMENDED FOCUS', 'NEXT REVIEW', 'EVIDENCE', 'DATA AVAILABILITY', 'SOURCE', 'HYPOTHETICAL SCENARIO', 'WHAT THE SCENARIO SHOWS'])
  return <div className="mt-5 space-y-2 text-sm leading-6 text-slate-800">{answer.split('\n').map((line, index) => {
    if (!line) return <div className="h-1" key={`space-${index}`} />
    if (headings.has(line)) return <h4 className="pt-2 text-xs font-bold uppercase tracking-[0.12em] text-slate-700" key={`${line}-${index}`}>{line}</h4>
    return <p className={line.startsWith('- ') || line.startsWith('  - ') ? 'pl-3 text-slate-700' : undefined} key={`${line}-${index}`}>{line}</p>
  })}</div>
}
...
function EvidenceCard({ evidence }: { evidence: AdvisorToolEvidence }) {
  ...
  if (evidence.tool === 'forecast') {
    ...
    return <Evidence title="FORECAST stored model result" lines={[`Model: ${String(result.model_name ?? 'Not available')}`, `Horizon: ${String(result.horizon_months ?? 'Not available')} months`, `Expected change versus the latest observed month: ${percentage(result.expected_change_pct)}`, ...(points ?? []).map((point) => `Projected medical cost for ${String(point.forecast_month ?? 'the forecast period').slice(0, 7)}: ${currency(point.predicted_cost)}`)]} />
  }
