...
def generate_drivers(dataset_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def get_drivers(dataset_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ownership_service.require_dataset(db, current_user, dataset_id)
    return driver_analysis_service.list(db, dataset_id)


@router.post("/alerts/generate", response_model=list[AlertResponse])
def generate_alerts(dataset_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ownership_service.require_dataset(db, current_user, dataset_id)
    return alert_service.generate(db, dataset_id)


@router.get("/alerts", response_model=list[AlertResponse])
def get_alerts(dataset_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ownership_service.require_dataset(db, current_user, dataset_id)
